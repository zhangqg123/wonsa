...
function initUpload(basePath){
    ...
    // Change this to the location of your server-side upload handler:
        var url = basePath+"/p3/xzkx/index.do?doUpload",
        uploadButton = $('<button/>')
...
//        	$("#imageHref").val(d.attributes.url);
        	$("#imageHref").val(d.attributes.fileKey);
  ...
  function showPic(path,id)
  {
	  try {
		  ...
		  {
			  var img = '<div id=\"'+id+'\"><img name=\"photo\" src=\"'+basePath+'/'+path+'\" width=\"230\" height=\"160\" id=\"'+path+'\"/>';
			  $('#files').append(img);
	      }
	  } catch (e) {
	  }
